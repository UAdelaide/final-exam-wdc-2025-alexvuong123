...
app.get('/api/walkers/summary', async (req, res) => {
  try {
    const [rows] = await db.execute(`SELECT Users.username AS walker_username, COUNT(WalkRatings.rating_id) AS total_ratings, AVG(WalkRatings.rating) AS average_rating,
      (SELECT COUNT(*) FROM WalkRequests JOIN WalkRequests ON WalkRequests.request_id = WalkRequests.request_id
      WHERE WalkRequests.walker_id = Users.user_id AND WalkApplications.status = 'completed') AS completed_walks
